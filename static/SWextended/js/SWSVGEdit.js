...
var SW_SVGCanvas = function(domSVG, swCanvas, domSVGPropertyToolsContainer)//, domDocumentToolsContainer)
{
	...
	if (!this.svgCanvas.hasTemplate())
	{
	    ...
	    var tmplateRect = this.svgCanvas.rect(0,0, 800,600, 0, 0).attr( {fill: 'white' } );
...
SW_SVGCanvas.prototype.drawCircle = function() {
	...
	this.tmpDrawing = this.svgCanvas.ellipse(tmpCord.x+(tmpCord2.x-tmpCord.x)/2, tmpCord.y+(tmpCord2.y-tmpCord.y)/2, (tmpCord2.x-tmpCord.x)/2, (tmpCord2.y-tmpCord.y)/2).attr({ stroke: '#123456', 'strokeWidth': 2,  fill: "white", "fill-opacity": 0});	
...
SW_SVGCanvas.prototype.drawRectangle = function() {
	...
	this.tmpDrawing = this.svgCanvas.rect(tmpCord.x, tmpCord.y, (tmpCord2.x-tmpCord.x), (tmpCord2.y-tmpCord.y), 0, 0).attr({ stroke: '#123456', 'strokeWidth': 2, fill: "white", "fill-opacity": 0.0});
...
SW_SVGCanvas.prototype.drawLine = function() {
	...
	this.tmpDrawing = this.svgCanvas.polyline(tmpCord.x, tmpCord.y, 
			tmpCord2.x, tmpCord2.y).attr({ stroke: '#123456', 'strokeWidth': 2, fill: "white", "fill-opacity": 0.0 });
...
SW_SVGCanvas.prototype.drawPolyline = function(evt) {
	...
	this.tmpDrawing = this.svgCanvas.polyline([drwingCordinates]).attr({ stroke: '#123456', 'strokeWidth': 2, fill: 'white', "fill-opacity": 0.0 });
...
SVGShape.prototype.createArcModifier = function (arcIndexInPath)
{
	...
	var arcResizeStart = function (mystartResizer, myendResizer, resizerType)
	{
		...
		origAngleBetweenArcPts = SW_SVG_UTIL.angleBetweenTwoLinesClockwise({x: curvePth.csX, y: curvePth.csY}, arcCenter, {x: curvePth.ceX, y: curvePth.ceY});
	...
	var arcResize = function (curvePath, arcCenter, L1, L2, mystartResizer, myendResizer, myrediusResizer, resizerType, dx, dy, x, y, evt)
	{
		...
		else if (resizerType == "CS")
		{
			...
			newCurvePath.csX=interSectionPoints[0].x;
			newCurvePath.csY=interSectionPoints[0].y;				
		...
		var newAngleBetweenArcPts = SW_SVG_UTIL.angleBetweenTwoLinesClockwise({x: newCurvePath.csX, y: newCurvePath.csY}, arcCenter, {x: newCurvePath.ceX, y: newCurvePath.ceY});
		
		if (origAngleBetweenArcPts == 180)
		{
			if (newAngleBetweenArcPts > 180 && curvePath.alf == 1)
				newCurvePath.alf = 1;
			else if (newAngleBetweenArcPts <= 180 && curvePath.alf == 0)
				newCurvePath.alf = 0;
			else 
				newCurvePath.alf = curvePath.alf == 0 ? 1 : 0;
		}
		else if (origAngleBetweenArcPts < 180)
		{
			if (newAngleBetweenArcPts <= 180)
				newCurvePath.alf = curvePath.alf;
			else 
				newCurvePath.alf = curvePath.alf == 0 ? 1 : 0;
		}			
		else // (origAngleBetweenArcPts > 180)
		{
			if (newAngleBetweenArcPts > 180)
				newCurvePath.alf = curvePath.alf;
			else 
		...
		this.displayShapeDitails(resizerType+","+newCurvePath.alf +";"+newAngleBetweenArcPts+  ","+ curvePath.alf + "," +origAngleBetweenArcPts);

		//this.displayShapeDitails(centerDiffX +","+ centerDiffY+","+newCurvePath.asf +";"+ curvePath.asf +","+ Math.round(newArcCenter[0].x)+","+ Math.round(newArcCenter[0].y)+ "," +  arcCenter.x+ ","+arcCenter.y +","+angleBetweenArcPts+ "," +origAngleBetweenArcPts);
...
SVGShape.prototype.createPointModifier = function()
{
	var meshLineHR = null;
	var meshLineVR = null;
	...
	var pathSegSupportsHorizontalResize = function(segmentType)
	{
		if (segmentType.toUpperCase() == "V") 
			return false;

		return true;
	}

	var pathSegSupportsVerticalResize = function(segmentType)
	{
		if (segmentType.toUpperCase() == "H") 
			return false;

		return true;
	}

	var IsPathSegRelativePath = function(segmentType)
	{
		if (segmentType.toUpperCase() == segmentType) 
			return false;

		return true;
	}

	var calculatePosistionForPathSegment = function (currentSegment, segmentTochange, currentSegmentIndex, newPoint, segmentsWithAbsoluteValue, indexOfPointToChange_X, indexOfPointToChange_Y, isPointBeforeCurrentPoint)
	{

			var pointValue = {x: null, y: null, lenOrPoint: null, type: currentSegment.type};

			if (currentSegment.type == "h"|| currentSegment.type == "H" || currentSegment.type == "v" || currentSegment.type == "V")
			{
				pointValue.lenOrPoint = currentSegment.values[0];
			}
			else 
			{
				pointValue.x = currentSegment.values[0];			
				pointValue.y = currentSegment.values[1];
			}

			var change ="Ty:"+ indexOfPointToChange_Y+ "Tx:"+ indexOfPointToChange_X;
			// To move x point
			if (currentSegmentIndex == indexOfPointToChange_X && indexOfPointToChange_X != -1)
			{
				if (!IsPathSegRelativePath(currentSegment.type) && !isPointBeforeCurrentPoint)
				{
					//Absolute path after selected segment, do not change
				}
				else if (pointValue.lenOrPoint == null)
				{
					if (!pathSegSupportsHorizontalResize(segmentTochange.type))
						pointValue.x = newPoint.x;								
					else if (!pathSegSupportsVerticalResize(segmentTochange.type))
				...
				else
				{
					...
					if (pathSegSupportsHorizontalResize(pointValue.type) && 
									(!pathSegSupportsHorizontalResize(segmentTochange.type) || IsPathSegRelativePath(pointValue.type)))
					{							
						if (isPointBeforeCurrentPoint)
						{	
					...
					else if (pathSegSupportsVerticalResize(pointValue.type) 
							         && (!pathSegSupportsVerticalResize(segmentTochange.type) || IsPathSegRelativePath(pointValue.type)))
					{
						if (isPointBeforeCurrentPoint)
						{
						...
						{
							var distY = origPointAbsoluteValueArray[1] - newPoint.y;//y
							pointValue.lenOrPoint = distY;
						}
					}
				}							
			...
			// To move Y point
			if (currentSegmentIndex == indexOfPointToChange_Y && indexOfPointToChange_Y != -1)
			{	
	...
	{		
		...
		else
		{
			...
			for (var currentSegmentIndex = 0; currentSegmentIndex < origSegments.length; currentSegmentIndex++)
			{
				var seg = origSegments[currentSegmentIndex];

				if (currentSegmentIndex == changePointAt)
				{
					if (seg.type == "h")
					{	
				...
				{		
					...
					{
						indexOfPointToChangeBeforeCurrentPoint_X = currentSegmentIndex;
					}
					else if (seg.type == "v" || seg.type == "V")
					{	
						indexOfPointToChangeBeforeCurrentPoint_Y = currentSegmentIndex;
					}
					else
					{
						indexOfPointToChangeBeforeCurrentPoint_X = currentSegmentIndex;
				...
				else if (currentSegmentIndex > changePointAt)
				{
						if (seg.type == "h" || seg.type == "H")
						{
							if (indexOfPointToChangeAfterCurrentPoint_X == -1)
								indexOfPointToChangeAfterCurrentPoint_X = currentSegmentIndex;
						}
						else if (seg.type == "v" || seg.type == "V")
						{	
							if (indexOfPointToChangeAfterCurrentPoint_Y == -1)
								indexOfPointToChangeAfterCurrentPoint_Y = currentSegmentIndex;
						}
						else
						{
							if (indexOfPointToChangeAfterCurrentPoint_X == -1)
			...
			for (var currentSegmentIndex = 0; currentSegmentIndex < origSegments.length; currentSegmentIndex++)
			{
				...
				if (currentSegmentIndex == changePointAt)
				{
					...
					if (!IsPathSegRelativePath(seg.type))
					...
					{ //current seg is relative point
							...
							else
							{
								if (IsPathSegRelativePath(seg.type))
								{
									var pointAbsoluteValueArray = segmentsWithAbsoluteValue[currentSegmentIndex];
									var distX = newPoint.x - pointAbsoluteValueArray[1];//x
									var distY = newPoint.y - pointAbsoluteValueArray[2]; //y
									d1 += seg.type + (seg.values[0]+ distX) + " " + (seg.values[1]+ distY);
								}
								else
								{	
									d1 += seg.type + newPoint.x + " " + newPoint.y;
...
SVGShape.prototype.addPolyLineDecorator = function()
{
	...
	var polylineDragger = this.createPointModifier();
...
SVGShape.prototype.addPathDecorator = function()
{
	...
	var shapeDragger = this.createPointModifier();
...
SVGShape.prototype.resetCurvePathCord = function (curvePath, indexInPath)
{
	...
	if (this.shape.type == "path")
	{
		...
		//this.displayShapeDitails(newSegment.type + d1);
